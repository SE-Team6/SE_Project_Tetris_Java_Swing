...
import static seoultech.se.tetris.config.ConfigBlock.RANDOM_CHAR;
...
//        StyleConstants.setAlignment(nextStyleSet, StyleConstants.ALIGN_CENTER);
    ...
    protected void generateNewBlock() {
        ...
        if(this.cnt >= 1 && itemNum >= 0&&itemNum<=10/*lineCount>0&&lineCount % 10 == 0*/){
            next = getRandomItemBlock();
            itemNum += 10;
            System.out.println("1");

        }else if(this.cnt>=1&&itemNum>10&&itemNum<=20)
        {
            next = getRandomBlock();
            this.cnt = 0;
            itemNum += 10;
